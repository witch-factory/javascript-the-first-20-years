...
ES1(§9)에 있었던 내장 메서드들은 본질적으로 메타-객체 프로토콜의 흔적이었다. 
...
ES1(§9)에 의해 제공된 내부 메소드는 본질적으로 잔존하는 메타오브젝트 프로토콜이었다. 다양한 종류의 내장 및 호스트가 제공하는 객체들 사이의 속성 접근의 관찰 가능한 의미론의 변화는 그들의 내부 메소드의 사양에서의 차이로 설명될 수 있다는 것이 의도였다. 하지만 ES2015 이전에는 내부 메소드 의미론이 불완전하고, 충분히 명시되지 않았으며, 일관되지 않게 사용되었다. 호스트 객체를 다루고, 이례적 객체의 자체 호스팅을 가능하게 하며, 객체-능력 막(Membranes)[Van Cutsem and Miller 2013]을 지원하기 위해, ES1–ES5 내부 메소드 설계는 완전히 명시된 MOP(메타오브젝트 프로토콜)로 변형되었다.