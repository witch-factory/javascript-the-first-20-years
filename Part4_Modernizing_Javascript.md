...
ES6는 사양에서 사용된 용어를 명확히 하고 업데이트할 기회를 제공했습니다. 주의가 필요했던 한 영역은 객체의 명명법이었습니다. JavaScript 1.0 구현은 ECMAScript 코드를 사용하여 생성할 수 있는 객체와 다양한 이례적인 방식으로 근본적인 객체 의미론이 달랐던 호스트 특정 및 JavaScript 엔진 특정 객체에 대한 접근을 JavaScript 프로그램에 제공했습니다. ES1 사양은 "객체", "네이티브 객체", "표준 객체", "내장 객체", "표준 네이티브 객체", "내장 네이티브 객체", "호스트 객체"라는 용어를 사용하여 객체가 구현될 수 있는 다양한 방식을 나타냈습니다. 이러한 지정 사이의 구분은 미묘했으며 특별히 유용하지 않았습니다. 이러한 카테고리 중 어느 것이 이례적인 객체 의미론을 허용하는지, 또는 JavaScript 프로그래머가 생성한 객체가 이 카테고리 중 어느 것에도 속하는지 정확히 명확하지 않았습니다.

ES6의 목표 중 하나는 대부분의 표준 라이브러리와 호스트 객체를 자바스크립트 코드를 사용하여 자체 호스팅할 수 있게 하는 것이었습니다. 자체 호스팅의 가능성을 감안할 때, 호스트 제공, 엔진 제공, 프로그램 제공 사이의 구분은 점점 중요성이 줄어들었습니다. 객체 간의 의미론적 차이는 그것들을 제공하는 주체나 구현에 사용된 기술보다 더 중요합니다.

필수적인 용어 필요성은 정상 의미론을 가진 객체와 비정상적(즉, 이례적인) 의미론을 가진 객체를 구분하는 것이었습니다. Douglas Crockford [TC39 2012b]는 Ecma의 최고 회원 등급의 이름을 빗대어 "보통 객체"를 자바스크립트 객체 리터럴이나 새로운 Object()를 사용하여 생성된 객체의 의미론을 가진 객체를 위한 용어로 제안했습니다. 의미론이 보통 객체 의미론과 어떤 방식으로든 다른 객체는 "이례적 객체"라고 불렸습니다.

보통 객체와 이례적 객체는 호스트, 엔진 또는 애플리케이션 프로그래머에 의해 제공될 수 있으며, 자바스크립트 또는 다른 언어를 사용하여 구현될 수 있습니다.